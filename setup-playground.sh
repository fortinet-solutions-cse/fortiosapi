...
install-packages()
{
   #  Go passwordless for sudo this is a dev playground DO NOT DO in Prod
    ...
    sudo apt -y install zfsutils-linux lxd virt-manager openvswitch-switch-dpdk juju python-openstackclient python-novaclient python-glanceclient python-neutronclient ubuntu-desktop chromium-browser vino python-pip zile htop||  exit 2
