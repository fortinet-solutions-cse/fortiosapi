...
python get-cloudify.py -e cfy_virtualenv --install-virtualenv --upgrade --version 3.4.1
#sudo pip install cloudify==4.0a10
...
$LXCm ping -c 4 docs.getcloudify.org
$LXCm yum update
...
$LXCm echo 'export JAVACMD=`which java`' | $LXCm tee -a /etc/environment
$LXCm sudo reboot
sleep 5
$LXCm ping -c 4 docs.getcloudify.org
...
cfy init -r
cfy bootstrap --install-plugins  cloudify-manager-blueprints/simple-manager-blueprint.yaml -i lxc-manager-blueprint-inputs.yaml || echo "error catched but keep going anyway"
